...
public:
	...
	std::string GetIP()
	{
		...
		if (getsockname(m_hSock, (sockaddr *)&addr, (int *)&addr_len) < 0) return "";
	...
	USHORT GetPort()
	{
		...
		if (getsockname(m_hSock, (sockaddr *)&addr, (int *)&addr_len) < 0) return 0;
	...
	bool Bind(USHORT nPort = 0, std::string ip = "")
	{
		...
		bind(m_hSock, (sockaddr *)&sockAddr, sizeof(sockaddr_in));
		return  true;
	}
