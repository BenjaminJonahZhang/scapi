...
#endif
#include <iostream>
#include "ScGarbledCircuitNoFixedKey.h"
...
(JNIEnv *env, jobject, jstring fileName, jboolean isFreeXor){
	...
	if (isFreeXor){
		garbledCircuit = new FastGarblingFreeXorHalfGatesFixedKeyAssumptions(str);
	}
	else{

		//the best implementation for non free xor is the 4 to 2 
		garbledCircuit = new FastGarblingFourToTwoNoAssumptions(str);
	}
...
(JNIEnv *env, jobject, jlong gbcPtr, jbyteArray garbledTables){
	...
	if (garbledCircuit->getIsFreeXor()){
		size = (garbledCircuit->getNumberOfGates() - garbledCircuit->getNumOfXorGates() - garbledCircuit->getNumOfNotGates())*2 * 16;
	}
	else{
			size = (((garbledCircuit->getNumberOfGates() - garbledCircuit->getNumOfXorGates() - garbledCircuit->getNumOfNotGates()) * 33) / 16 + 1 + garbledCircuit->getNumOfXorGates()) * 16;
	}
	...
	memcpy(garbledCircuit->getGarbledTables(), carr, size);
